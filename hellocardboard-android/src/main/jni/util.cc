...
/**
 * Initializes the mesh from an in-memory OBJ buffer.
 * This is a simplified parser for sub-OBJs generated by FlexieAnimator.
 * Supports 'v' (vertices), 'vt' (texture UVs), and 'f' (faces with v/t format).
 *
 * @param buffer Pointer to the raw OBJ data.
 * @param size   Length of the buffer in bytes.
 * @param position_attrib OpenGL position attribute location.
 * @param uv_attrib       OpenGL UV attribute location.
 * @return true on success.
 */
bool TexturedMesh::InitializeFromBuffer(const char* buffer, int size,
  ...
  std::vector<GLfloat> temp_pos, temp_norm, temp_uv;
  std::vector<GLushort> v_indices, n_indices, t_indices;

  // Minimal parser for sub-OBJs generated by FlexieAnimator
  std::string line;
  while (std::getline(ss, line)) {
    ...
    if (type == "v") {
      ...
      std::string v1, v2, v3;
      ls >> v1 >> v2 >> v3;
      auto parse_face = [&](std::string s) {
        size_t slash1 = s.find('/');
        int vi = std::stoi(s.substr(0, slash1)) - 1;
        int ti = std::stoi(s.substr(slash1 + 1)) - 1;
        vertices_.push_back(temp_pos[vi * 3]);
        vertices_.push_back(temp_pos[vi * 3 + 1]);
        vertices_.push_back(temp_pos[vi * 3 + 2]);
        uv_.push_back(temp_uv[ti * 2]);
        uv_.push_back(temp_uv[ti * 2 + 1]);
        indices_.push_back(static_cast<GLushort>(indices_.size()));
      };
      parse_face(v1);
      parse_face(v2);
      parse_face(v3);
